...
from dataset_utils.data_utils import load_as_float
from dataset_utils.raw import raw
...
class ValidationFlowKitti2015MV(data.Dataset):
    ...
    def __init__(
        ...
        raw_root=None,
        example=0,
        true_motion=False,  # pylint: disable=unused-argument
    ):
        self.root = Path(root)
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(gt_flow_path):
            ...
            gtDisp = np.array(gtDisp, dtype=float) / 256.0
        else:
...
class ValidationFlowKitti2015(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt_flow_path = self.root.joinpath(
            self.phase, "flow_occ", str(index).zfill(6) + "_10.png"
        )
        gt_disp_path = self.root.joinpath(
            self.phase, "disp_occ_0", str(index).zfill(6) + "_10.png"
        )
        ...
        if self.mapping[index] is not None:
            ...
            dataset = raw(
                ...
                seq,
                frames=range(
                    self.mapping[index]["Frame"] - 1, self.mapping[index]["Frame"] + 2
                ),
                origin=1,
            )
            calib = {}
...
class ValidationFlowKitti2012(data.Dataset):
    ...
    def __getitem__(self, index):
        tgt_img_path = self.root.joinpath(
            self.phase, "colored_0", str(index).zfill(6) + "_10.png"
        )
        ref_img_past_path = self.root.joinpath(
            self.phase, "colored_0", str(index).zfill(6) + "_11.png"
        )
        ref_img_future_path = self.root.joinpath(
            self.phase, "colored_0", str(index).zfill(6) + "_11.png"
        )
